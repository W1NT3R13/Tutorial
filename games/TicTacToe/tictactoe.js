...
const refreshStats = () => {
  ...
  sessionStorage.setItem(`${isVsAi ? "vsAiStats" : "vsPlayerStats"}`, stringStats);
};
...
const checkEndOfGameRemoveListeners = () => {
  if (isGameOver()) {
    isDraw
      ? stats.draws++
      : mark === "X" ? stats.p1Wins++ : stats.p2Wins++;
...
const handleOpenModal = (event) => {
  ...
  if (event === "Stats") {
    ...
    const tableHeaders = Array.from(document.querySelectorAll(".statsTableHead"))
